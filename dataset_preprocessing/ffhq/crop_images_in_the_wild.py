...
if __name__ == '__main__':
    ...
    img_files = sorted([x for x in os.listdir(args.indir) if x.lower().endswith(".png") or x.lower().endswith(".jpg") and 'mirror' not in x.lower()])
    lm_files = sorted([x for x in os.listdir(lm_dir) if x.endswith(".txt") and 'mirror' not in x.lower()])

    img_files = [img_files[i] for i in range(args.rank, len(img_files), args.world_size)]
    lm_files= [lm_files[i] for i in range(args.rank, len(lm_files), args.world_size)]
    ...
    for img_file, lm_file in zip(img_files, lm_files):
        if not os.path.exists(os.path.join(out_dir, img_file.split(".")[0] + ".png")):
