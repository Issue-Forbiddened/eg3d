...
def generate_images():
    ...
    target_decoder=deepcopy(G.decoder)
    ...
    if not os.path.exists(os.path.join(args.output_dir,'scale_factor.json')):
        if accelerator.is_main_process:
            total_batch=100000
            minibatch=64
            sample_count=0
            ...
            # 计算标准差
            scale_factor = torch.sqrt(variance).mean(dim=[1, 2]) # (96,)
            scale_factor=1/scale_factor
            scale_factor = scale_factor.cpu().numpy().tolist()
            with open(os.path.join(args.output_dir,'scale_factor.json'),'w') as f:
                json.dump(scale_factor,f)
                
    accelerator.wait_for_everyone()
    # read scale_factor
    with open(os.path.join(args.output_dir,'scale_factor.json'),'r') as f:
        scale_factor=json.load(f)
    scale_factor=torch.tensor(scale_factor,device=device,dtype=weight_dtype).reshape(1,96,1,1)
    
    if scale_factor.isnan().any():
        raise ValueError('scale_factor has nan value')
    ...
    for epoch in tqdm(range(first_epoch, args.num_train_epochs),desc='epoch:'):
        ...
        for step in tqdm(range(epoch_size),desc='step:'):
            ...
            planes_scaled=torch.tanh(planes*scale_factor)*scale_factor_tanh
            ...
            if global_step%log_step_interval==0:
                if accelerator.is_main_process:
                    with torch.no_grad():
                        ...
                        eg3d_distilled_output_=eg3d_distilled_output['image']
                        ...
                        if args.use_ema:  
                            ...
                            eg3d_distilled_output_ema=G.render_from_planes(camera_params,planes_scaled,decoder=target_decoder)
